...
if ($(".in-index").length){
    ...
    fetch("https://www.janheder.ga/znacka/")
        .then((response) => response.text())
        .then(loadBrands)
}
*/
...
if (paginationNav !== null) {
    function refactorPagi() {
        ...
        for (var i = 1; i <= max; i++) {
            ...
            li.classList.add('pagination__link'); // základní třída na li

            if ((current - i) > 2 || (i - current) > 1) {
                if (i !== 1 && i !== max) {
                    li.classList.add('hidden'); // skryté vzdálené stránky
                }
            }
        ...
        // Přidání tlačítek předchozí / další
        if (current > 1) {
...
// Add event listener to each select element with class .hidden-split-parameter to listen for changes
