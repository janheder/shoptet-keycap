function konfigurator236(){
    ...
    const filterSection = document.querySelector('.filter-section-parametric-id-236');
      ...
      if (labels.length === 0) {
        ...
        function updateDisabledOptions() {
          ...
          [...profileSelect.options].forEach(option => {
            ...
            option.disabled = ![...rimSet].some(rim => combinations.has(combination + rim));
          });
          ...
          if (profileSelect.value && profileSelect.selectedOptions[0].disabled) {
            ...
            console.log('Neplatná možnost v profileSelect, nastaveno zpět na "Vybrat"');
          }
        }
        ...
        function updateRimOptions(selectedWidth, selectedProfile) {
          ...
          if (rimSelect.value && rimSelect.selectedOptions[0].disabled) {
            ...
            console.log('Neplatná možnost v rimSelect, nastaveno zpět na "Vybrat"');
          }
        }
    
        // Funkce pro zaškrtnutí příslušného input
        function selectInput() {
          const selectedWidth = widthSelect.value;
          const selectedProfile = profileSelect.value;
          const selectedRim = rimSelect.value;
    
          // Pokud jsou všechny selekty vybrány, najdeme odpovídající input a zaškrtneme ho
          if (selectedWidth && selectedProfile && selectedRim) {
            const selectedCombination = `${selectedWidth}/${selectedProfile}/${selectedRim}`;
            console.log(`Hledám input s value: ${selectedCombination}`);
    
            // Najdeme všechny checkboxy v sekci a odškrtneme je
            const checkboxes = filterSection.querySelectorAll('input[type="checkbox"]');
            checkboxes.forEach(checkbox => checkbox.checked = false);
    
            // Najdeme input s odpovídajícím value a simulujeme na něm kliknutí
            const inputToCheck = filterSection.querySelector(`input[value="${selectedCombination}"]`);
            if (inputToCheck) {
              inputToCheck.click();  // Simulujeme kliknutí na checkbox
              console.log(`Kliknutím zaškrtnut input s value: ${selectedCombination}`);
            } else {
        ...
        // Přidáme event listener na kliknutí na tlačítko pro zaškrtnutí inputu
        button.addEventListener('click', selectInput);
      }
    }
